...
namespace KInspector.Reports.ContentTreeConsistencyAnalysis.Models
{
    public class VersionHistoryMismatchResult
    {
        ...
        {
            var hasAtLeastOneNullValue = versionHistoryXmlValue is null || coupledDataColumnValue is null;
            ...
            else
            {
                switch (fieldType)
                {
                    ...
                    default:
                        DocumentValue = coupledDataColumnValue?.ToString() ?? string.Empty;
                        VersionHistoryValue = versionHistoryXmlValue;
                        break;
                }
            }
        ...
        private void ProcessDecimalValues(string? versionHistoryXmlValue, object? coupledDataColumnValue)
        {
            ...
            var versionHistoryValue = decimal.Parse(versionHistoryXmlValue ?? string.Empty);
        ...
        private void ProcessDateTimeValues(string? versionHistoryXmlValue, object? coupledDataColumnValue)
        {
            ...
            DateTime coupledDataDateTime = DateTime.Parse(coupledDataColumnValue?.ToString() ?? string.Empty);
