...
namespace Kentico.KInspector.Web
{
    ...
    public class ExportController : ApiController
    {
        ...
        public HttpResponseMessage GetModuleExport([FromUri] IEnumerable<string> moduleNames,
            [FromUri] InstanceConfig config, [FromUri] string exportModuleCodeName)
        {
            if (config == null)
            {
            ...
            var module =
                ExportModuleLoader.Modules.FirstOrDefault(m => m.ModuleMetaData.ModuleCodeName == exportModuleCodeName);
            if (module == null)
            {
            ...
            catch (Exception e)
            {
                return Request.CreateResponse(HttpStatusCode.InternalServerError,
                    $"Error in processing modules. Error message: {e.Message}");
            }
        }
    }
}