	...
	WHILE @@FETCH_STATUS = 0
		...
		IF (@WebPartsCount > 0)
			BEGIN
				...
				INSERT @MY_Documents SELECT DISTINCT NodeAliasPath, DocumentCulture, SiteName FROM View_CMS_Tree_Joined LEFT JOIN CMS_WorkflowStep ON View_CMS_Tree_Joined.DocumentWorkflowStepID=CMS_WorkflowStep.StepID LEFT JOIN CMS_Culture ON View_CMS_Tree_Joined.DocumentCulture = CMS_Culture.CultureCode WHERE (NodeAliasPath LIKE N'/%') AND (DocumentPageTemplateID = @pageTemplateID OR NodeTemplateID = @pageTemplateID OR NodeWireframeTemplateID = @pageTemplateID)
